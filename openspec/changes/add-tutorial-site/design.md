...
콘텐츠 세부 사항 두 가지다. 스펙·접근·태스크 분해를 바꾸지 않으므로 초안을 쓰고 검토받는다.

1. **"권한 인증 커맨드"의 정확한 의미** — 사용자가 준 목차에서 Node.js 설치 스텝에 딸린 항목이다. npm 전역 설치 권한 문제 해결(macOS의 소유권 변경, Windows의 실행 정책)로 해석해 OS 분기로 초안을 작성한다. 다른 의미였다면 해당 스텝의 본문만 교체하면 된다.
2. **챕터 3 "클로드 코드에서 프로젝트 열어보기"의 본문** — 하위 스텝 없이 챕터 제목만 주어졌다. "클로드 코드 실행 → 폴더 열기 → 권한 승인 → 첫 질문" 흐름으로 초안을 작성한다.
