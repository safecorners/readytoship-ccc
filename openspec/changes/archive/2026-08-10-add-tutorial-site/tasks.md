...
- [x] 9.1 `npm run build` 성공 및 `/guides/claude-code-basics`가 정적 렌더(SSG)로 표시되는지 빌드 로그 확인
- [x] 9.2 콘텐츠 검증 확인 — 스텝 식별자를 일부러 중복시키고 빌드가 실패하는지, 오류 메시지가 원인을 짚는지 확인 후 되돌리기
- [x] 9.3 콘텐츠 검증 확인 — 스텝 본문 파일을 일시 제거하고 빌드가 실패하는지 확인 후 되돌리기
- [x] 9.4 `npm run lint` 통과
- [x] 9.5 스텝 체크 → 챕터 카운터 즉시 증가, 새로고침 후 유지
- [x] 9.6 스텝 없는 챕터가 자체로 체크되고 전체가 6개로 집계되는지 확인 (검수 시점 기준. 이후 페이즈 10에서 챕터 3이 스텝으로 쪼개져 8개가 됐다)
- [x] 9.7 진행 이력이 있는 상태로 새로고침 시 `0/6 → 3/6` 깜빡임이 없는지 확인 (페이즈 10 이후에도 8개 기준으로 재확인)
- [x] 9.8 브라우저 콘솔에 hydration 경고가 없는지 확인
- [x] 9.9 두 탭에서 한쪽 체크 시 다른 탭에 반영되는지 확인
- [x] 9.10 저장된 값을 일부러 손상시킨 뒤 페이지가 정상 렌더되는지 확인
- [x] 9.11 명령어 복사 → 붙여넣기 결과가 표시된 문자열과 일치하는지 확인
- [x] 9.12 학습 경로의 단계 클릭 → 상세의 해당 챕터가 펼쳐진 상태로 이동하는지 확인
- [x] 9.13 키보드만으로 전체 순회, 포커스 링이 항상 보이는지 확인
- [x] 9.14 375 / 768 / 1280px 폭에서 가로 스크롤이 없는지 확인
- [x] 9.15 시각 언어 점검 — 볼드 디스플레이, 채도 높은 버튼 채움, 파스텔의 비장식 용도가 어디에도 없는지 확인
- [x] 9.16 Open Question 2건(권한 명령의 의미, 챕터 3 본문)의 초안을 사용자에게 검토 요청

## 10. 검토 반영 — 챕터 3을 스텝으로 분해

9.16의 검토 결과다. Open Question 1은 초안대로 확정됐고, Open Question 2에서 챕터 3을 세 스텝으로 쪼개기로 하면서 체크 단위가 6개에서 8개로 늘었다.

- [x] 10.1 `design.md` Open Questions를 확정 결정으로 고쳐 쓰고, 결정 3(암묵 정규화)을 남기는 근거 기록
- [x] 10.2 `proposal.md`·`specs/guide-progress`의 체크 단위 수치를 8개로, `specs/guide-content`에 첫 가이드가 암묵 경로를 쓰지 않게 됐음을 명시
- [x] 10.3 `guide.mdx` — 챕터 3에 스텝 3개 선언 (프로젝트 폴더 열기 / 폴더 접근 권한 승인하기 / 첫 질문 던지기), 요약 문구의 "여섯 단계"를 고침
- [x] 10.4 `steps/open-project-in-claude-code.mdx`를 세 파일로 분해
- [x] 10.5 랜딩 문구의 "여섯 단계" 수정
- [x] 10.6 `npm run build` / `npm run lint` 통과, 전체 체크 단위가 8개로 집계되는지 확인
